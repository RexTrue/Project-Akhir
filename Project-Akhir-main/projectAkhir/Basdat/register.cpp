...
void Register::on_pushButton_clicked()
{
    ...
    if(Nama.isEmpty() || NIK.isEmpty() || UserName.isEmpty() || Alamat.isEmpty() || NmrHP.isEmpty() || Password.isEmpty()){
        ...
        msg.setStyleSheet("width:500px; font-size:18px; font-weight:bold; text-align:center; justify-content:center;");
        msg.exec();
    }
    else{
        ...
        if(!NamaCheck(Nama)){
            QMessageBox::warning(ui->frame, "", "Nama hanya boleh karakter (A-Z, a-z) dan spasi");
        }
        else if(resNIK == 0){
           QMessageBox::warning(ui->frame, "", "NIK berisi 16 angka");
        }
        else if(resNIK == 1){
           QMessageBox::warning(ui->frame, "", "Maaf NIK sudah terdaftar");
        }
        else if(resUsername == 0){
           QMessageBox::warning(ui->frame, "", "Username hanya boleh karakter (A-Z, a-z), angka, garis bawah (_)");
        }
        else if(resUsername == 1){
           QMessageBox::warning(ui->frame, "", "Maaf Username sudah terdaftar");
        }
        else if(!NmrHPCheck(NmrHP)){
           QMessageBox::warning(ui->frame, "", "Nomor HP berisi angka dan panjang 10 sampai 13");
        }
        else if(!tglLahirCheck(TglLahir)){
           QMessageBox::warning(ui->frame, "", "Maaf umur anda belum mencukupi");
        }
        else{
           QMessageBox::information(ui->frame, "", "Pendaftaran Berhasil");
           QSqlQuery query;
